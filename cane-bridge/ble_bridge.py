...
class Bridge:
    ...
    def on_phone_text(self, text):
        ...
        # v3: single-letter motor commands from the Shepherd app.
        #   L = steer left, R = steer right, S = straight, X = stop
        # Forwarded to the App Lab app which drives the Motor Modulino.
        cmd = text.strip().upper()
        if cmd in ("L", "R", "S", "X"):
            resp = http_post_json("/api/motor", {"cmd": cmd})
            if resp is not None:
                self.tx.send_line(json.dumps({"cmd": cmd}, separators=(",", ":")))
            else:
