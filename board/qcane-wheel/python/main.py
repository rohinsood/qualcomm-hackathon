...
# Dodge flip: a RIGHT turn that has run for ~this much arc with the obstacle
# still inside the presence threshold has exhausted that side — swing LEFT
# instead. The wheel has no yaw sensor, so arc is a time proxy: full-scale
# spin yaws the cane at roughly WHEEL_YAW_DEG_PER_S (estimate — tune on the
# real cane), so 60 deg ~= 3 s. The timer re-arms whenever the wheel leaves
# RIGHT or the path clears, and any later phone/dashboard command still wins
# (last writer, as always).
DODGE_FLIP_DEG = 60.0
WHEEL_YAW_DEG_PER_S = 20.0
DODGE_FLIP_AFTER_S = DODGE_FLIP_DEG / WHEEL_YAW_DEG_PER_S
...
_right_dodge_t0 = None


def _dodge_flip():
    """Flip a stuck RIGHT dodge to LEFT once ~DODGE_FLIP_DEG of arc has been
    spent without clearing the obstacle. Runs at the watchdog's 4 Hz."""
    global _right_dodge_t0
    with _lock:
        stuck = _state["motor"] == DIRECTIONS["right"] and _state["present"]
    if not stuck:
        _right_dodge_t0 = None
        return
    now = time.monotonic()
    if _right_dodge_t0 is None:
        _right_dodge_t0 = now
        return
    if now - _right_dodge_t0 >= DODGE_FLIP_AFTER_S:
        _right_dodge_t0 = None
        _log_event(f"Right dodge spent ~{DODGE_FLIP_DEG:.0f}° with object "
                   "still ahead — flipping left")
        _set_wheel(DIRECTIONS["left"], DASHBOARD_SPEED, "dodge flip")
