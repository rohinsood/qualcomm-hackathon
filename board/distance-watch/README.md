...
**This is now the merged cane app** — v3's `qcane-wheel`, which absorbed the
original `distance-watch` app, carried into this repo under the
`distance-watch` name:
...
```bash
python3 <repo>/board/distance-watch/host/qcane_btd.py        # foreground
python3 <repo>/board/distance-watch/host/qcane_btd.py -v     # with debug logs
```
...
cp <repo>/board/distance-watch/host/qcane-btd.service ~/.config/systemd/user/
...
`applyMotor()` is called from `loop()` on every change, never from the Bridge
...
```bash
python3 host/qcane_btd.py --selftest        # always full scale — the sketch ignores any percent argument
```
...
```
distance-watch/
├── app.yaml                 one brick (arduino:web_ui, the dashboard) — Bluetooth is not one, and could not run here anyway
├── assets/                  QCane Link dashboard (served on :7000)
