...
import com.example.qhackgps.scan.Detection
import java.util.Locale

/**
 * Rolling, thread-safe scene state shared between the camera scan and the
 * conversational companion: the latest detections and guidance verdict,
 ...
 *
 * Adapted from v3: the verdict there came from the full guidance engine;
 * here [Severity] mirrors the thirds decision (STRAIGHT = CLEAR, a dodge =
 * CAUTION, STOP = DANGER) and everything is empty while the SCAN toggle is
 * off — the companion then honestly reports that the camera sees nothing.
 *
...
class SceneBlackboard {
    ...
    private var severity = Severity.CLEAR
    private var nearestLabel: String? = null
    private var nearestDistance: Float? = null
    private val events = ArrayDeque<Event>()

    /** One-line navigation progress; null when no destination is set. */
    ...
    @Synchronized
    fun updateGuidance(severity: Severity, nearestLabel: String?, nearestDistance: Float?) {
    ...
    fun digest(nowMs: Long): String = buildString {
        ...
        if (label != null && dist != null && severity != Severity.CLEAR) {
