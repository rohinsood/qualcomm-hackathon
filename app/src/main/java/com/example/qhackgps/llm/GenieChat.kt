...
/**
 * Conversational companion: a persistent Qwen3.5-2B session on the Hexagon
 * NPU, ported from the v3 branch. In this app the NPU is uncontended — the
 * camera scan's vision runs GPU-first (see scan/OrtSessions) — so the
 * accelerator split v3 designed for finally holds with nothing parked.
 *
 * Each user turn is grounded with a scene digest from [SceneBlackboard],
 * and the reply streams out sentence-by-sentence so the voice starts
 * speaking after the first sentence, not after the whole reply. A safety
 * alert can cut a reply off at any time via [requestStop]; the interruption
 * is replayed to the model on the next turn so the companion can pick the
 * thread back up — like a friend who paused mid-sentence to say "car on
 * your right".
 */
class GenieChat {
    ...
    companion object {
        ...
        private val SYSTEM_PROMPT = """
            You are Lighthouse, a friendly walking companion for a blind or low-vision pedestrian, speaking out loud through their phone. You are a normal, knowledgeable assistant: general questions — facts, capitals, math, advice, small talk — get a direct answer from your own knowledge, exactly like any assistant.
