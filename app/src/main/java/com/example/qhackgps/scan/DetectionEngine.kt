 ...
 * Qualcomm QNN Execution Provider so inference lands on the Adreno GPU or
 * Hexagon NPU (see [OrtSessions] for the tiered fallback and the S25 Ultra
 ...
 *  2. bundled asset `yolov8_det.onnx`
 */
class DetectionEngine {
    ...
    fun initialize(context: Context): Boolean {
        val modelBytes = loadModelBytes(context) ?: run {
            Log.e(TAG, "No model found: bundle $MODEL_FILE as an asset or adb-push it.")
            return false
        }
