...
/**
 * Screen-thirds obstacle steering, ported from the v3 branch.
 *
 ...
 * "stop", then "straight" when the way clears): this class only decides.
 *
 * The [update] segmentation input is kept from v3 for the logic and its
 * tests, but this app runs detection-only — callers pass null and the
 * seg-less fallback rules apply (sides start from [UNKNOWN_SIDE_SCORE],
 * the middle blocks on near objects alone).
 *
...
class ThirdsGuidance {
     ...
     *   (16 columns), or null when segmentation has no opinion — always
     *   null in this app; see the class doc.
     */
